        ...
        if self.paginering['dummy']:
            ...
            if isinstance( self, nvdbFagdata): 
                ...
                self.data = self.anrope( '/'.join(('vegobjekter', str(self.objektTypeId) )), 
                    parametre=parametre )
                self.antall = self.data['metadata'].get('antall',0)
        ...
        if ignorerGeometriFeil or 'geometri' in feat.keys():
            ...
            if vegsegmenter: 
                for seg in feat['vegsegmenter']:
                    ...
                    if vr in seg.keys() and 'vegsystem' in seg[vr].keys():
                    ...
                    if 'startposisjon' in seg.keys() and 'sluttposisjon' in seg.keys() and "lengde" in seg.keys():
                    ...
                    if vr in seg.keys() and 'strekning' in seg[vr].keys() and 'adskilte_løp' in seg[vr]['strekning']:
                        s2['adskilte_lop'] = seg[vr]['strekning']['adskilte_løp']


                    delkeys = [ 'strekning', 'kryssystem', 'sideanlegg']
                    for hvaslag in delkeys: 
                        if vr in seg.keys() and hvaslag in seg[vr].keys(): 
